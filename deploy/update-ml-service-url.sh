...
# Update ECS Task Definition with ML Service Private IP
# This script gets the ML service private IP and updates the task definition
...
echo -e "${GREEN}[1/5]${NC} Getting ML service task ARN..."
...
echo -e "${GREEN}[2/5]${NC} Getting ML service private IP..."
...
echo -e "${GREEN}[3/5]${NC} Updating task definition file..."
# Update the task definition JSON file
sed "s/ML_SERVICE_IP/$ML_PRIVATE_IP/g" ecs-task-definition.json > ecs-task-definition-updated.json

echo -e "${GREEN}[4/5]${NC} Registering new task definition with AWS ECS..."
...
echo -e "${GREEN}[5/5]${NC} Updating backend service to use new task definition..."
...
echo "rm ecs-task-definition-updated.json"
